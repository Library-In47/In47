...
export class CartComponent {
  ...
  private calcularTotal(): number {
    return this.carritoService.elementosCarrito.reduce(
      (total, producto) =>
        total + parseFloat(producto.libro.precio) * producto.cantidad,
      0
    );
  }
  ...
  finalizarCompra() {
    ...
    console.log(compra);
    this.carritoService.enviarCompra(compra).subscribe(
      (response) => {
        console.log('Orden procesada exitosamente');
        Swal.fire('Orden procesada exitosamente', '', 'success').then(() => {
